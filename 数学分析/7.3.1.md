...
$\displaystyle \left| \frac{u_{n+1}}{u_{n}(x)} \right|=\left| \frac{x^{n+1}}{n+1}\cdot \frac{n}{x^{n}} \right|=\frac{n}{n+1}|x|\to |x|$
...
$\displaystyle \left| \frac{u_{n+1}}{u_{n}} \right|=\left| \frac{2^{n+1}x^{n+1}}{(n+1)!}\cdot\frac{n!}{2^{n}x^{n}} \right|=\frac{2x}{n+1}\to 0$ 收敛
...
比值和根值只是一个充分性的办法, 不能直接用 $\displaystyle \sum_{n=1}^{\infty}\left| \frac{a_{n+1}}{a_{n}} \right|$ 证明相等, 但是可以用来证明前者较小.
