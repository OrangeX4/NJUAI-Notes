...
cetanswer(F(x, y), ($\mathcal{T}, \mathcal{A}$)) = { (John_Lennon, John_Lennon), (John_Lennon, Paul_McCartney), (John_Lennon, Ringo_Starr), (Paul_McCartney, John_Lennon), (Paul_McCartney, Paul_McCartney), (Paul_McCartney, Ringo_Starr), (Ringo_Starr, John_Lennon), (Ringo_Starr, Paul_McCartney), (Ringo_Starr, Ringo_Starr) }
...
- cetanswer(r(x, y) ∧ Y (y), $\mathcal{A}$) = { (Jay_Chou, Jolin_Tsai), (Jolin_Tsai, Stefanie_Sun), (Stefanie_Sun, Jay_Chou), (Jolin_Tsai, Jolin_Tsai), (Stefanie_Sun, Stefanie_Sun) }
- cetanswer(∃y(r(x, y) ∧ Y (y)), $\mathcal{A}$) = { Jay_Chou, Jolin_Tsai, Stefanie_Sun }
- cetanswer(∃x, y(r(x, y) ∧ r(y, x)), $\mathcal{A}$) = "Yes"
- cetanswer(∃z, w(r(x, y) ∧ r(y, z) ∧ r(z, x) ∧ r(z, w) ∧ W(w)), $\mathcal{A}$) = { (Jay_Chou, Jolin_Tsai), (Stefanie_Sun, Jay_Chou), (Jolin_Tsai, Jolin_Tsai), (Jolin_Tsai, Stefanie_Sun) }
...
Let $k = 2$ and then a graph is 2-colorable if and only if its vertices having same color can be taken as disjoint sets. Thus, A graph is 2-colorable if and only if it is 2-colorable, which can be also solved in PTime. So it can not prove the proposition.
